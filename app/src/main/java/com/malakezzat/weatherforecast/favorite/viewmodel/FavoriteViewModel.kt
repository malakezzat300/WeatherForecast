...
class FavoriteViewModel(val weatherRepository: WeatherRepository) : ViewModel() {
    ...
    private val _favoriteList = MutableSharedFlow<List<FavoriteDB>>()
    val favoriteList: SharedFlow<List<FavoriteDB>> get() = _favoriteList
    ...
    private suspend fun getFavoriteData() {
        weatherRepository.getFavoriteData().collect { favoriteList ->
            _favoriteList.emit(favoriteList)
        }
    }
